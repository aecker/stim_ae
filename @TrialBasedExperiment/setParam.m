...
for i = 1:length(params)
    ...
    elseif(paramType == 2) % String or array of strings
        ...
        try
            strLen = pnet(con,'read',1,'uint16'); % Ignore this value
            values = pnet(con,'readline');
            values = textscan(values,'%s');

            % Save vector of cells, where cells contain strings
            e.params.(params{i}) = reshape(values{1},e.paramSizes.(params{i}),[]);
        catch
            err = lasterror;
            fprintf('Had problem in setParam while setting %s.\nError message: %s\n',params{i}, err.message);
            success = false;
        end
    end
end
...
e.randomization = feval(e.params.randomization{1},nVals,e.params.numTrials);
