...
if((targetSetup == 1) | (targetSetup == 4))
    ...
    incorrectColor = (curParams.targetBColor*curParams.incorrectIntensity + (1-curParams.incorrectIntensity)*curParams.bgColor)/1;
    %incorrectColor = (curParams.targetBColor)/1;
elseif((targetSetup == 2) | (targetSetup == 3))
    distribution = eval(curParams.distributionB{1});
    correctColor = curParams.targetBColor;
    %incorrectColor = (curParams.targetAColor)/1;
    incorrectColor = (curParams.targetAColor*curParams.incorrectIntensity + (1-curParams.incorrectIntensity)*curParams.bgColor)/1;
else
    error('No distribution selected');
end
cdf = cumsum(distribution)/sum(distribution);
 
if((targetSetup == 1) | (targetSetup == 3))
    leftTargetColor = correctColor;
    rightTargetColor = incorrectColor;
else
    leftTargetColor = incorrectColor;
    rightTargetColor = correctColor;
end    
...
while GetSecs < endTime
    ...
    % Give him the answer
    %Screen('glPoint',win,correctColor,halfWidth,halfHeight-targetOffset/3,targetDiameter);    
